...
export function QuranScreen() {
  ...
  return (
    <AppScreen>
      <AppHeader title="Kur'an" subtitle="Sureleri oku, ayetleri işaretle ve kaldığın yerden sakin şekilde devam et." />
      ...
      {!isLoading && filtered.length === 0 && (view === "all" || favoriteVerses.length === 0) ? (
        <EmptyState message={view === "favorites" ? "Henüz favori sure veya ayet yok." : "Sure bulunamadı."} />
      ) : null}

      {filtered.map((chapter) => {
        const favorite = favoriteChapterIds.includes(chapter.id);
        return (
          <ContentCard key={chapter.id}>
            <View style={styles.chapterRow}>
              <Pressable accessibilityRole="button" onPress={() => router.push(`/quran/${chapter.id}`)} style={styles.chapterCopy}>
                ...
                <Text style={[theme.typography.section, { color: theme.colors.text }]}>{chapter.translation}</Text>
                <Text style={[styles.chapterMeta, { color: theme.colors.textMuted }]}>
