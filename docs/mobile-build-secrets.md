...
Not: `EXPO_PUBLIC_*` değerleri uygulama paketine gömülür; klasik anlamda gizli anahtar değildir. Yine de repo içine yazılmaması ve Actions secret/variable olarak tutulması daha düzenlidir.
...
| `EXPO_PUBLIC_ALADHAN_SCHOOL` | `0` | Varsayılan asr ayarı. |
| `EXPO_PUBLIC_PURCHASE_PROVIDER` | `mock` | Canlı satın alma açılmadan `mock` kalmalı. `store` seçeneği ürün mimarisi için hazırdır, fakat native IAP modülü şu an devre dışıdır. |
...
RevenueCat kullanılmaz. Sükût Ayrıcalık için ürün id'leri ve paywall mimarisi hazırdır. Android build uyumluluğu için sorun çıkaran native IAP paketi şu an devre dışıdır; canlı mağaza satın alma açılmadan önce Expo SDK ile uyumlu bir IAP modülü yeniden etkinleştirilmelidir.
